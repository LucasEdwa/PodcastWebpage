...
const ContactForm = () => {
  ...
  return (
    <div id="contact" className='bg-custom-gradient transition-500 py-10 lg:p-20 flex items-center justify-between m-6 gap-2'>
      <h3 className='text-2xl shadow-2xl border-blue-500 border border-3 p-2 text-white w-2/4'>{t('contact_us')}</h3>
      <div className='w-full'>
        <ul className='flex flex-col gap-5 text-left'>
          {contactMethods.map((method, index) => (
            <li key={index}>
              <a href={method.href} className='text-white underline text-sm'>
                {method.label}
              </a>
            </li>
          ))}
